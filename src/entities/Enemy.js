...
export default class Enemy extends Phaser.GameObjects.Container {
  ...
  applyDifficultyMultipliers() {
    ...
    this.data.health = Math.round(
      this.data.health * this.difficultyMultipliers.enemyHealthMultiplier
    );

    // Apply speed multiplier
    this.data.speed = Math.round(
      this.data.speed * this.difficultyMultipliers.enemySpeedMultiplier
    );

    // Apply reward multiplier
    this.data.reward = Math.round(
      this.data.reward * this.difficultyMultipliers.enemyRewardMultiplier
    );
  }
  ...
  updateSlowEffects(time) {
    ...
    this.slowEffects = this.slowEffects.filter(
      (effect) => effect.endTime > time
    );
  ...
  getPathProgress() {
    ...
    const currentSegmentLength = Phaser.Math.Distance.Between(
      ...
      this.path[this.currentPathIndex + 1].x,
      this.path[this.currentPathIndex + 1].y
    );
