...
export default class MapSelectScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.DIFFICULTY_KEYS.forEach((key, idx) => {
            ...
            let btnColor;
            switch(key) {
                case 'EASY': btnColor = 0x00aa00; break;     // Green
                case 'NORMAL': btnColor = 0x0088cc; break;   // Blue
                case 'HARD': btnColor = 0xaaaa00; break;     // Yellow
                case 'EXPERT': btnColor = 0xdd6600; break;   // Orange
                case 'INSANE': btnColor = 0xcc0000; break;   // Red
                default: btnColor = 0x888888;
            }

            // Difficulty button
            const btn = this.add.rectangle(x, 470, 160, 60, btnColor, 0.7)
                .setInteractive()
                .setOrigin(0.5);
            
            // Difficulty name
            this.add.text(x, 470, difficultyData.name, {
                fontSize: '24px',
                fill: '#fff'
            }).setOrigin(0.5);
        ...
        // Start game button
        this.startButton = this.add.rectangle(640, 580, 240, 80, 0x008800, 0.8)
            .setInteractive()
            .setOrigin(0.5);
        
        this.add.text(640, 580, 'Start Game', {
            fontSize: '32px',
            fill: '#fff'
        }).setOrigin(0.5);
